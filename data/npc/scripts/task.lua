...
function getPlayerTask(cid, monster)`
    playerTask = mysqlQuery("SELECT `deaths`, `objective` FROM `player_task` WHERE `player_id` = \"" .. cid .. "\" AND `monster_id` = \"" .. monster .. "\" ", "bank_balance")
        return playerTask;
end

function updateTask(cid, monster)
	playerTask = getPlayerTask(cid, monster);
	if playerTask.deaths < playerTask.objective then
		mysqlQuery('UPDATE `player_task` SET `deaths`+="1" WHERE `´player_id` = "' .. cid .. '" AND `monster_id` = "' .. monster .. '"', 'UPDATE')
		return TRUE
	end
end
